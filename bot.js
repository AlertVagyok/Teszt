...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    ...
    .setAuthor("RipperZ RP Szerver Státusz", "https://media.discordapp.net/attachments/892860882747547678/904701429921488917/download.jpg")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '🟢 Elérhető' : '🟢 Elérhető')
    .setTimestamp(new Date())
    .addField('**Bemutatkozó:** ','  \`\`\`A RipperZ RolePlay egy kezdetleges szerver ahol arra törekszünk, hogy a játékosoknak az igényeit kielégítsük és olyan szerveren játszhassanak ahol a játékosok szavai igen is számítanak. \`\`\`')
    .addField('**Szerver IP cím:** ','  \`\`\`Jelenleg Nem Elérhető \`\`\`')
    .addField('**TeamSpeak 3 szerver:** ','  \`\`\`Jelenleg Nem Elérhető \`\`\`')
    if (STATUS !== undefined)
    {
      embed.addField(':warning: Jelenlegi Szerver Státusz:',`${STATUS}\n\u200b\n`);
  ...
  const offline = function() {
    ...
    .setColor(0xff0000)
    .addField('**Szerver Státusz** ',' \`\`\`Offline\`\`\`')
    sendOrUpdate(embed);
    LAST_COUNT = null;
  };
  ...
  const updateMessage = function() {
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        let embed = UpdateEmbed()
        .addField('**Szerver Státusz**: ',' \`\`\`Online\`\`\`', true)
        .addField('**Szerver Restartok:** ','  \`\`\`12:00, 00:00 \`\`\`')
        .addField('**Elérhető Játékosok:** ', `\`\`\`${players.length}\`\`\``,true);
        // .addField('\u200b','\u200b\n\u200b\n',true);
        if (players.length > 0) {
          // method D
          const fieldCount = 3;
